...
def test_warehouse_separates_paid_and_canceled_orders(
    tmp_path: Path,
    valid_frames: dict[str, pd.DataFrame],
) -> None:
    valid_frames["orders"] = pd.DataFrame(
        [
            {
                "order_id": "O1",
                "customer_id": "C001",
    ...
    valid_frames["order_items"] = pd.DataFrame(
        [
            {
                "order_id": "O1",
                "product_id": "P1",
    ...
    marts = build_warehouse(valid_frames, tmp_path / "warehouse.duckdb")
